...
class Illustrator:
    ...
    def draw(self, object_data, frame, drone_velocity, battery_level, braking=False):
        ...
        y_velocity_string = 'Y VELOCITY: ' + str(drone_velocity[2])
        cv2.putText(frame, y_velocity_string, (40, 80), font, 1, (0, 0, 153), 2, cv2.LINE_AA)
        yaw_velocity_string = 'YAW VELOCITY: ' + str(drone_velocity[3])
        cv2.putText(frame, yaw_velocity_string, (40, 120), font, 1, (0, 0, 153), 2, cv2.LINE_AA)
        if braking:
